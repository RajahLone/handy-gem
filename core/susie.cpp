...
bool CSusie::ContextSave(FILE *fp)
{
  ...
  if(!afwrite(&mTMPADR,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mTILTACUM,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mHOFF,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mVOFF,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mVIDBAS,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mCOLLBAS,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mVIDADR,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mCOLLADR,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSCBNEXT,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSPRDLINE,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mHPOSSTRT,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mVPOSSTRT,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSPRHSIZ,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSPRVSIZ,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSTRETCH,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mTILT,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSPRDOFF,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSPRVPOS,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mCOLLOFF,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mVSIZACUM,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mHSIZACUM,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mHSIZOFF,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mVSIZOFF,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mSCBADR,sizeof(UUWORD),1,fp, 2)) return 0;
  if(!afwrite(&mPROCADR,sizeof(UUWORD),1,fp, 2)) return 0;
  
  if(!afwrite(&mMATHABCD,sizeof(TMATHABCD),1,fp, 4)) return 0;
  if(!afwrite(&mMATHEFGH,sizeof(TMATHEFGH),1,fp, 4)) return 0;
  if(!afwrite(&mMATHJKLM,sizeof(TMATHJKLM),1,fp, 4)) return 0;
  if(!afwrite(&mMATHNP,sizeof(TMATHNP),1,fp, 4)) return 0;
  
  if(!afwrite(&mSPRCTL0_Type,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL0_Vflip,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL0_Hflip,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL0_PixelBits,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mSPRCTL1_StartLeft,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL1_StartUp,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL1_SkipSprite,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL1_ReloadPalette,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL1_ReloadDepth,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL1_Sizing,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCTL1_Literal,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mSPRCOLL_Number,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRCOLL_Collide,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mSPRSYS_StopOnCurrent,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_LeftHand,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_VStretch,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_NoCollide,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_Accumulate,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_SignedMath,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_Status,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_UnsafeAccess,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_LastCarry,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_Mathbit,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mSPRSYS_MathInProgress,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mSUZYBUSEN,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mSPRINIT,sizeof(TSPRINIT),1,fp, 0)) return 0;
  
  if(!afwrite(&mSPRGO,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mEVERON,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(mPenIndex,sizeof(UBYTE),16,fp, 0)) return 0;
  
  if(!afwrite(&mLineType,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mLineShiftRegCount,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mLineShiftReg,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mLineRepeatCount,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mLinePixel,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mLinePacketBitsLeft,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mCollision,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mLineBaseAddress,sizeof(ULONG),1,fp, 4)) return 0;
  if(!afwrite(&mLineCollisionAddress,sizeof(ULONG),1,fp, 4)) return 0;
  
  if(!afwrite(&mJOYSTICK,sizeof(TJOYSTICK),1,fp, 0)) return 0;
  if(!afwrite(&mSWITCHES,sizeof(TSWITCHES),1,fp, 0)) return 0;
...
bool CSusie::ContextLoad(LSS_FILE *fp)
{
  ...
  // assert(sizeof(UUWORD)    == 2);
  // assert(sizeof(TMATHABCD) == 4);
  // assert(sizeof(TMATHEFGH) == 4);
  // assert(sizeof(TMATHJKLM) == 4);
  // assert(sizeof(TMATHNP)   == 4);
  // assert(sizeof(TJOYSTICK) == 1);
  // assert(sizeof(TSWITCHES) == 1);
  // assert(sizeof(TSPRINIT)  == 1);
  
  if(!lss_read(teststr,                 sizeof(char),     19, fp, 0)) return 0;
  if(strcmp(teststr,"CSusie::ContextSave")!=0) return 0;
  
  if(!lss_read(&mTMPADR,                sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mTILTACUM,              sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mHOFF,                  sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mVOFF,                  sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mVIDBAS,                sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mCOLLBAS,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mVIDADR,                sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mCOLLADR,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSCBNEXT,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSPRDLINE,              sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mHPOSSTRT,              sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mVPOSSTRT,              sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSPRHSIZ,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSPRVSIZ,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSTRETCH,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mTILT,                  sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSPRDOFF,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSPRVPOS,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mCOLLOFF,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mVSIZACUM,              sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mHSIZACUM,              sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mHSIZOFF,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mVSIZOFF,               sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mSCBADR,                sizeof(UUWORD),    1, fp, 2)) return 0;
  if(!lss_read(&mPROCADR,               sizeof(UUWORD),    1, fp, 2)) return 0;
  
  if(!lss_read(&mMATHABCD,              sizeof(TMATHABCD), 1, fp, 4)) return 0;
  if(!lss_read(&mMATHEFGH,              sizeof(TMATHEFGH), 1, fp, 4)) return 0;
  if(!lss_read(&mMATHJKLM,              sizeof(TMATHJKLM), 1, fp, 4)) return 0;
  if(!lss_read(&mMATHNP,                sizeof(TMATHNP),   1, fp, 4)) return 0;
  
  if(!lss_read(&mSPRCTL0_Type,          sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL0_Vflip,         sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL0_Hflip,         sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL0_PixelBits,     sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mSPRCTL1_StartLeft,     sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL1_StartUp,       sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL1_SkipSprite,    sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL1_ReloadPalette, sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL1_ReloadDepth,   sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL1_Sizing,        sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCTL1_Literal,       sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mSPRCOLL_Number,        sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRCOLL_Collide,       sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mSPRSYS_StopOnCurrent,  sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_LeftHand,       sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_VStretch,       sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_NoCollide,      sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_Accumulate,     sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_SignedMath,     sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_Status,         sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_UnsafeAccess,   sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_LastCarry,      sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_Mathbit,        sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mSPRSYS_MathInProgress, sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mSUZYBUSEN,             sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mSPRINIT,               sizeof(TSPRINIT),  1, fp, 0)) return 0;
  
  if(!lss_read(&mSPRGO,                 sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mEVERON,                sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(mPenIndex,               sizeof(UBYTE),    16, fp, 0)) return 0;
  
  if(!lss_read(&mLineType,              sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mLineShiftRegCount,     sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mLineShiftReg,          sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mLineRepeatCount,       sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mLinePixel,             sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mLinePacketBitsLeft,    sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mCollision,             sizeof(ULONG),     1, fp, 4)) return 0;
  
  if(!lss_read(&mLineBaseAddress,       sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mLineCollisionAddress,  sizeof(ULONG),     1, fp, 4)) return 0;
  if(!lss_read(&mJOYSTICK,              sizeof(TJOYSTICK), 1, fp, 0)) return 0;
  if(!lss_read(&mSWITCHES,              sizeof(TSWITCHES), 1, fp, 0)) return 0;
...
ULONG CSusie::PaintSprites(void)
{
  ...
  do
  {
    ...
    if(!mSPRCTL1_SkipSprite)
    {
      ...
      {
        sprintf(say_buffer, "* CSusie:PaintSprites() - Rendered Sprite %03d",sprcount); say(say_buffer);
    ...
    if(sprcount>4096)
    {
      ...
      sprintf(say_buffer, "* CSusie:PaintSprites(): Single draw sprite limit exceeded (>4096). The SCB is most likely looped back on itself. Reset/Exit is recommended\n"); say(say_buffer);
...
inline void CSusie::ProcessPixel(ULONG hoff,ULONG pixel)
{
  switch(mSPRCTL0_Type)
  {
    ...
    case sprite_noncollide:
      if(pixel!=0x00) ZWritePixel(hoff,pixel);
      break;
    ...
    case sprite_xor_shadow:
      if(pixel!=0x00)
      {
        ZWritePixel(hoff,ZReadPixel(hoff)^pixel);
      }
      if(pixel!=0x00 && pixel!=0x0e)
      {
        if(!mSPRCOLL_Collide && !mSPRSYS_NoCollide && pixel!=0x0e)
...
inline void CSusie::ZWritePixel(ULONG hoff,ULONG pixel)
{
...
inline ULONG CSusie::ZReadPixel(ULONG hoff)
{
  ULONG scr_addr=mLineBaseAddress+(hoff/2);
...
inline ULONG CSusie::LineInit(ULONG voff)
{
  ...
  if(voff>101)
  {
    sprintf(say_buffer, "* CSusie::LineInit() Out of bounds (voff)\n"); say(say_buffer);
    voff=0;
  }
