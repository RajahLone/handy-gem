...
public:
  ...
  inline bool ContextSave(FILE *fp)
  {
    ...
    if(!afwrite(&mX,sizeof(ULONG),1,fp, 4)) return 0;
    if(!afwrite(&mY,sizeof(ULONG),1,fp, 4)) return 0;
    if(!afwrite(&mSP,sizeof(ULONG),1,fp, 4)) return 0;
    if(!afwrite(&mPS,sizeof(ULONG),1,fp, 4)) return 0;
    if(!afwrite(&mPC,sizeof(ULONG),1,fp, 4)) return 0;
    if(!afwrite(&mIRQActive,sizeof(ULONG),1,fp, 4)) return 0;
    return 1;
  }
  ...
  inline bool ContextLoad(LSS_FILE *fp)
  {
    ...
    if(!lss_read(&mA,sizeof(ULONG),1,fp, 4)) return 0;
    if(!lss_read(&mX,sizeof(ULONG),1,fp, 4)) return 0;
    if(!lss_read(&mY,sizeof(ULONG),1,fp, 4)) return 0;
    if(!lss_read(&mSP,sizeof(ULONG),1,fp, 4)) return 0;
    if(!lss_read(&mPS,sizeof(ULONG),1,fp, 4)) return 0;
    if(!lss_read(&mPC,sizeof(ULONG),1,fp, 4)) return 0;
    if(!lss_read(&mIRQActive,sizeof(ULONG),1,fp, 4)) return 0;
    PS(mPS);
    return 1;
  }
